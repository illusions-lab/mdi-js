...
| `parse_mdi_syntax`（Rust） | `parse_document` / `parse_output` | 回傳舊 `MdiSyntaxDocument`，沒有 spans、front matter、一般 Markdown nodes；詳見 [Document IR](/zh-tw/core/document-ir/#過渡期的-mdisyntaxdocument-形狀)。 |
...
IR 所有 span 都是 UTF-8 byte offset。不要未經明確轉換就視為宿主語言 character index；請見[診斷](/zh-tw/core/diagnostics/#spans精確來說)與[Python](/zh-tw/bindings/python/#ir-version-與-utf-8-byte-spans)。

## Remark adapter：今日僅單向

`@illusions-lab/mdi-remark` 將真實 Rust output 轉成 `mdast`，但編輯後的 `mdast` tree round-trip 回 `.mdi` 尚未套用 Rust 的 recommended-form normalization（例如 `《《...》》` → `[[em:...]]`）。詳見 [Remark adapter](/zh-tw/ecosystem/remark/#目前實作狀態今天是單向的)。
...
- [Rust Core API](/zh-tw/core/rust-api/#尚未實作)
