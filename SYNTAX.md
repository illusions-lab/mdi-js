...
**Normative in 2.0:** escape processing runs **once, first**, before any MDI inline parsing, and behaves identically in the editor (remark) path and the export path. An escaped character never participates in any MDI delimiter match.  
**2.0 での規範:** エスケープ処理はすべての MDI インライン解析に**先立って一度だけ**実行され、エディタ（remark）経路とエクスポート経路で同一に動作します。エスケープされた文字はいかなる MDI 区切り文字のマッチにも参加しません。
