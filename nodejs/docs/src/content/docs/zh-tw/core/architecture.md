...
除了標示 Planned 的箭頭外，以上關係今天都可驗證。`@illusions-lab/mdi`（JavaScript）、PyPI 上的 `illusion-markdown`（Python）以及 `@illusions-lab/mdi-remark` 都呼叫同一份 Rust 程式碼；請見[JavaScript 綁定](/zh-tw/bindings/javascript/)、[Python 綁定](/zh-tw/bindings/python/)與[Remark / mdast 轉接器](/zh-tw/ecosystem/remark/)。
...
| Swift binding（UniFFI/C ABI） | **Planned。**[`swift/README.md`](https://github.com/illusions-lab/MDI/blob/main/swift/README.md) 明確寫著尚未實作。 |
